...
`;async function g(a){try{let b=await a.json(),c=b.accountId?.trim(),g=b.apiToken?.trim(),h=b.projectName?.trim()||`relay-${Date.now().toString(36)}`;if(!c||!g)return d.NextResponse.json({error:"Cloudflare Account ID and API Token are required"},{status:400});let i=`https://api.cloudflare.com/client/v4/accounts/${c}/workers/scripts/${h}`,j=new FormData;j.append("index.js",new Blob([f],{type:"application/javascript+module"}),"index.js"),j.append("metadata",new Blob([JSON.stringify({main_module:"index.js",compatibility_date:"2024-03-20",observability:{enabled:!0}})],{type:"application/json"}),"metadata.json");let k=await fetch(i,{method:"PUT",headers:{Authorization:`Bearer ${g}`},body:j});if(!k.ok){let a=await k.json().catch(()=>({}));return console.error("Cloudflare upload error:",a),d.NextResponse.json({error:a.errors?.[0]?.message||"Failed to upload Worker to Cloudflare"},{status:k.status})}let l=await fetch(`${i}/subdomain`,{method:"POST",headers:{Authorization:`Bearer ${g}`,"Content-Type":"application/json"},body:JSON.stringify({enabled:!0})});if(!l.ok){let a=await l.json().catch(()=>({}));console.error("Cloudflare subdomain enable error:",a)}let m="",n=await fetch(`https://api.cloudflare.com/client/v4/accounts/${c}/workers/subdomain`,{method:"GET",headers:{Authorization:`Bearer ${g}`,"Content-Type":"application/json"}});if(n.ok){let a=await n.json();a.result&&a.result.subdomain&&(m=`https://${h}.${a.result.subdomain}.workers.dev`)}if(!m)return d.NextResponse.json({error:"Worker deployed but failed to retrieve workers.dev subdomain. Make sure you have setup a workers.dev subdomain in Cloudflare Dashboard."},{status:400});let o=await (0,e.ZO)({name:h,proxyUrl:m,type:"cloudflare",noProxy:"",isActive:!0,strictProxy:!1});return d.NextResponse.json({proxyPool:o,deployUrl:m},{status:201})}catch(a){return console.log("Error deploying Cloudflare relay:",a),d.NextResponse.json({error:a.message||"Deploy failed"},{status:500})}}},94735:a=>{a.exports=require("events")},94755:(a,b,c)=>{c.d(b,{A:()=>i});var d=c(77598);let e={randomUUID:d.randomUUID},f=new Uint8Array(256),g=f.length;var h=c(50122);let i=function(a,b,c){if(e.randomUUID&&!b&&!a)return e.randomUUID();var i=a,j=c;let k=(i=i||{}).random??i.rng?.()??(g>f.length-16&&((0,d.randomFillSync)(f),g=0),f.slice(g,g+=16));if(k.length<16)throw Error("Random bytes length must be >= 16");if(k[6]=15&k[6]|64,k[8]=63&k[8]|128,b){if((j=j||0)<0||j+16>b.length)throw RangeError(`UUID byte range ${j}:${j+15} is out of buffer bounds`);for(let a=0;a<16;++a)b[j+a]=k[a];return b}return(0,h.k)(k)}}};var b=require("../../../../webpack-runtime.js");b.C(a);var c=b.X(0,[4741,1813,4884],()=>b(b.s=44907));module.exports=c})();